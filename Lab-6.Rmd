...
We also need to find the mean of the sample:
...
$$Z_{obt} = \frac{73-71}{.66} = 3.03$$
Then we can look in the Z-Table for the $Z_{crit}$ of .05 is 1.64. 
...
```{r}
set.seed(3)
music_study<-data.frame(Metal=round(rnorm(10,85,5),digits = 0),Classical=round(rnorm(10,78,8),digits=0),Difference=c("+","+","+","-","+","+","+","+","-","+"))
music_study
```
...
```{r}
set.seed(3)
music_study<-data.frame(Metal=round(rnorm(10,85,5),digits = 0),Classical=round(rnorm(10,78,8),digits=0),Metal_Diff_Classical= c(8,15,14,-1,7  ,9,15,18,-9,11))
music_study
music_study
```

So we have two minuses and 8 pluses. What is the probability associated with this outcome with a sample of 10? In other words, think of this as $p(8)$ which necessaricly includes $p(9)$ and $p(10)$. 



```{r}
library(BSDA)
SIGN.test(music_study$Metal_Diff_Classical,alternative = "greater",md=0)
...
Now, let's go backwards a bit: When we calculated a z-test, we did it by "hand" and then by computer. Now let us peroform the same sign test by "hand".
...
We have learned about vectors and dataframees. We will now learn about matrices. A matrix is a set of numbers that do sometihng. 
...
```
Once you have a matrix you can refer to a specific data point using `[,]`. The way this works is that anything inside of the left bracket (`[`) will refer to the row. Anything inside of thr right bracket (`]`) will refer to the column. 
...
So far in this class, we have looked at datasets, and performed analsyes. The next step is to report these findings. 

All of psychology uses APA stye writing. Within this writing style there is a special style to report results. 

From this point forward, on your homeworks if you do any analyses and are asked to report your findings, you *must* use proper statistical formatting in order to get all of the points. 

The Z-Test deals with significnce, as does the Sign Test. 
