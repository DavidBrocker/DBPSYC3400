...
5. Below are two plots. Of the two, which one do you thing shows a more apparent relationship or pattern?
...
###New Material: Mean, Standard Deviation, Z-Score, Summation, Stem and Leaf

You will see the formula for the arithmetic mean in several differnt ways. Here are just a few. 
...
1. Find the mean and standard deviation for the following dataset:
datset1 = {10,63,51,24,87,42}

```{r}
datset1 = c(10,63,51,24,87,42)
mds = round(mean(datset1), digits = 2)
sdds=round(sd(datset1), digits = 2)
```
...
stem(x, scale = .5)
```
The first stem and leaf plot is the deafult you will get if you just type in `stem(x)`.
The second stem and leaf plot separates each leaf making the list twice as long. 
The third stem and leaf plot uses half the spacing as the first, so the scale is about half as long. 
